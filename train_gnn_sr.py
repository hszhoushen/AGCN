...
from model.network import AFM, PAFM
...
def main():
    ...
    if (args.fusion == False):
        ...
        audio_classifier = Audio_Fusion_Classifier(args).cuda()
    # fusion model
    else:
    ...
    if args.status == 'test':
        ...
        AFM_net.load_state_dict(checkpoint['audio_net_state_dict'])
        audio_gcn_model.load_state_dict(checkpoint['audio_gcn_model_state_dict'])
        ...
        test_losses, test_acc1, test_acc5 = validate(args, AFM_net, audio_gcn_model, audio_classifier,
                                                     val_loader, criterion, 0)
    ...
    if args.status == 'draw':
        ...
        # added
        for param in AFM_net.parameters():          # freeze netT
            param.requires_grad = False
        for param in audio_gcn_model.parameters():  # freeze netT
            param.requires_grad = False
        ...
        with torch.no_grad():
            for i in range(len(img_file_lst)):
                ...
                img_output, resnet_output = AFM_net(img)
                ...
                print('fused_output:', fused_output.shape, fused_output)
...
def train(args, AFM, audio_gcn_model, audio_classifier, data_loader, optimizer, criterion, epoch):
...
def validate(args, AFM, audio_gcn_model, audio_classifier, data_loader, criterion, epoch):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    with torch.no_grad():
        for i, (image, label, path) in enumerate(data_loader):
            ...
            if (args.fusion == False):
                ...
                gcn_output, rows, columns = audio_gcn_model(audio_output)
