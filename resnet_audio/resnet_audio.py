...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=527, zero_init_residual=False,
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))                             # changed (6, 4)
    ...
    def _forward_impl(self, x):
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        # print('max pool, x:', x.shape)


        x = self.layer1(x)
        # print('layer 1, x:', x.shape)

        x = self.layer2(x)
        # print('layer 2, x:', x.shape)
        Fea2 = x

        x = self.layer3(x)
        # print('layer 3, x:', x.shape)
        Fea3 = x

        x = self.layer4(x)
        # print('layer 4, x:', x.shape)
        Fea4 = x
        ...
        fc_rep = x # batch_size * 2048

        x = self.fc(x)
        x = self.sigmoid(x)

        # set the x in the range of min 1e-7 to max 1-1e-7
        x = torch.clamp(x, 1e-7, 1 - 1e-7)

        return fc_rep, Fea2, Fea3, Fea4
    ...
    def predict(self, x, batch_size = 100):
        ...
        for i in range(0, len(x), batch_size):
            with torch.no_grad():
                ...
                current_re = [var.data.cpu().numpy() for var in output]
                result.extend(current_re)
...
def resnet18(num_classes=50, pretrained=False, progress=True, **kwargs):
...
def resnet50(num_classes=50, pretrained=False, progress=True, **kwargs):
